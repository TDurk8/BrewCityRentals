	...
	if ($num > 0) { // If it ran OK, display all the records.
		echo "<br><div><b>Your search returned $num entries.</b></div>";
			if($_SESSION['role']=='admin'){
			  echo "<div><a text-align='center' href=searchform_admin.php>Search Again</a></div>";
			}
			else{
				echo"<div><a text-align='center' href=searchform.php>Search Again</a></div>";
			}
			  echo("</br>");
		...
		while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
			...
			echo "<div class='resultcenter'>".$row['runtime']." min</div>";
				if($_SESSION['role']=='admin'){
					...
					echo "<div><a href=rent.php?id=".$row['movie_id']."><img src=../images/rentnow.png alt=rentnow></a></div>"; 
					echo "<div></div>"; 
				}
